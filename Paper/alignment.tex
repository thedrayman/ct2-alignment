...
\begin{figure}
	\centering
	\includegraphics[width=0.45\textwidth]{images/SWTab.png}
	\caption{Filled-in Smith-Waterman matrix with traceback\cite{ibm}}
	\label{fig:watermansmith}
\end{figure}

To illustrate the use of the algorithm we consider two sequences \texttt{GCCCTAGCG} and \texttt{GCGCAATG} to be aligned, like shown in Fig. \ref{fig:watermansmith}.

\begin{enumerate}
\item The first column and row of the matrix should be initialized with zeros.
\item Beginning with the first index \texttt{(0,0)} the score is calculated iteratively with the help of recurrences. The score for \texttt{(G,G)} results in the value 1, because there is a match, whereas for \texttt{(G,C)} it results in 0, because the characters don't match (so the recurrence is \texttt{gap=-1}) and because the value is now negative, it is set to 0 according to the algorithm rule.
...
\item By reading from the matrix the result for the local alignment is indeed \texttt{GCG}.
\end{enumerate}
...
\begin{enumerate}
	\item First, the data sets are sorted by a timestamp and reduced to two columns, like presented in Table \ref{fig:csv_sorted}. 
...
Meanwhile, the values for the compressed sequence aggravate in terms of accuracy when the search sequence length is increased. However multiple advantages are given when using this method. Trough compressing the sequence, the redundancy of pointless character repitions is avoided so the actual prediction accuracy of useful characters is overall improved. Though it may seem, that the accuracy for uncompressed sequences is a little higher than for the compressed one, we should consider that it does not tell the real usefullness of the prediction because of random time intervals. So even if the accuracy for compressed sequences is lower, it is much more precise in that way and also faster regarding the sequence length.
