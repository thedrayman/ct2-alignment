...
\begin{enumerate}
	...
	\item After applying the \algoSW algorithm, we receive \texttt{B} as the predicted next character of the sequence because it follows after the first found character \texttt{A} in the history sequence. Again, the algorithm operates best on much longer sequences.
\end{enumerate}
...
The data, that is provided for us by three different people, compirises a time period from one and up to several month.

To evaluate the accuracy of the prediction, we try to make predictions on existent data to compare it to available data sets. From the whole sequence, we use a certain amount of data to be our test sequence. Hence the rest is the history sequence from which we extract the search sequence like before. We define all the characters after the search sequence (which are now located in the test sequence) as the \textit{expected prediction}. Now, we apply the \algoSW algorithm to the search and history sequence to obtain the possible prediction. If it is found, we compare it to the expected prediction. This evaluation method is shown in Fig. \ref{fig:diagram_test} .
...
Furthermore, we observe the impact of the search sequence length \textit{k} and the prediction sequence length \textit{l}. Dabei fällt auf, dass
