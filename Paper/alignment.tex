...
For location prediction individual datasets of the .csv file are sorted by the \texttt{timestamp}. The \texttt{location} defines the location name of the user while \texttt{logitude}, \texttt{latitude} and \texttt{altitude} represent the coordinates. Additionally, the \texttt{accuracy} describes the precision of the location. For our approach we need merely the description in a correct and chronological order.
...
The algorithm that is suited best for this kind of problem is the \algoSW because it can be utilized on sequences with different lengths to find the smallest possible match locally. Another benefit is that the sequences musn't be identical so that even similar sequences can be found as well. Algorithms like Needleman-Wunsch and Hirschberg are not suitable for it because those can only calculate the biggest, most similar sequence globally.
...
\item Beginning with the first index \texttt{(0,0)} the score is calculated iteratively with the help of reccurencies. The score for \texttt{(G,G)} results in the value 1, because there is a match, whereas for (G,C) it results in 0, because the characters don't match (so the reccurency is \texttt{gap=-1}) and because the value is now negative, it is set to 0 according to the algorithm rule.
...
The data, that is provided for us by three different people, compirises a time period from up to several month.

To evaluate the accuracy of the prediction, we try to make predictions on existent data to compare it to available data sets. From the whole sequence, we use a certain amount of data to be our test sequence. Hence the rest becomes the history sequence from which we extract the search sequence like before. We define all the characters after the search sequence (which are now the first few characters of the test sequence) as the \textit{expected prediction}. Now, we apply the \algoSW to the search and history sequence to obtain the \textit{possible prediction}. If this is found, we compare it to the expected prediction to verify the correctness of the process. This evaluation method is depicted in Fig. \ref{fig:prediction_concept}. 
