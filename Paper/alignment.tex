...
\title{Using Alignment Algorithm for Location Prediction on Google Location Data}
...
The benefit of it is that Google Location History data \cite{37522} is gathered automatically by every mobile device that uses Google services. The daily logged data contain accurate position coordinates and timestamps right up to location names and inaccuracies.

\begin{table}[h]
	\caption{Example of collected Google Location Data}
	\resizebox{0.49\textwidth}{!}{%home:700, work:500, friend:800, sport:300
		\begin{tabular}{l*{4}{c}r}
			timestamp & accuaracy & longitude & latitude & altitude & location \\ 
			\hline		
...
The data originates from the Google Location project \cite{google-location} and is available as a Comma-separated-values-file (CSV-file) that is represented in extracts in the following Table \ref{fig:csv}.

For location prediction individual datasets of the CSV-file are sorted by the \texttt{timestamp}. The \texttt{location} describes the location name of the user while \texttt{logitude}, \texttt{latitude} and \texttt{altitude} represent the coordinates. Additionally, the \texttt{accuracy} specifies the location precision. For our approach we need merely location names in a correct and chronological order.

The idea now is to predict the next location of a person with the previous location data. Now we consider the data as a long sequence like depicted in Fig. \ref{fig:diagram}. Then a search sequence with the last \textit{x} characters is extracted from it. Finally the alignment is applied to the entire sequence to find the most similar local sequence. All following characters form the prediction sequence of length \textit{l}.

The algorithm that is suited best for this kind of problem is the \algoSW because it can be utilized for sequences with different lengths to find the smallest possible match locally. Another benefit is that the sequences mustn't be identical so that even similar sequences can be found as well. Algorithms like Needleman-Wunsch and Hirschberg are not suitable for it because those can only calculate the biggest, most similar sequence globally.
...
\begin{figure}
	\centering
	\includegraphics[width=0.45\textwidth]{images/SW-BABA.png}
	\caption{Filled-in Smith-Waterman matrix with traceback\cite{baba}}
	\label{fig:watermansmith}
\end{figure}

To illustrate the use of the algorithm we consider two sequences \texttt{ACBACCAB} and \texttt{ACA} to be aligned, like shown in Fig. \ref{fig:watermansmith}.

\begin{enumerate}
\item The first column and row of the matrix should be initialized with zeros.
\item Beginning with the first empty slot in the upper left corner the score is calculated iteratively with the help of recurrences. The score for \texttt{(A,A)} results in the value 2, because there is a match, whereas for \texttt{(A,C)} it results in 1, because the characters don't match (so the recurrence is \texttt{gap=-1}) and because the previous value is 2, the recurrence is subtracted.
\item If the matrix is calculated completely, the highest value, in this case it is 5, is found.
\item Now, the traceback algorithm searches for the right path backwards diagonally to the north west until the value 0. As of now, the path has the length of 5. As said before, the calculated value 0 does not count. That means the path length is 4.
\item By reading from the matrix the result for the local alignment is \texttt{A-CA}, because the algorithm found a gap.
\end{enumerate}
...
In Chapter \ref{section:conception} we already explained, that we require for the alignment only timestamps and descriptions from the data.
...
The next step will be to define \textit{x} characters at the end of the history sequence. Now, both sequences are handed over to the \algoSW in order to compute the score matrix. In our program, the algorithm operates on the default recurrences values. A matrix cell with the highest score and with the indices of the found local sequence is expected as output so that we can  get the values of the history sequence directly. If the matrix contains more than one highest score, the first found one is taken. All the characters with the selected length \textit{l} can now be read off the history sequence. We finally obtain the predicted sequence.
...
Let us assume that the extract from a CSV-file consists of unsorted data like shown in Table \ref{fig:csv}.

\begin{enumerate}
	\item First, the data sets are sorted by a timestamps and reduced to two columns, like presented in Table \ref{fig:csv_sorted}. 
	\item Each location name now represents an individual character from the alphabet. This is shown in Fig. \ref{fig:csv_converted}.
	\item When every character is arranged into a character string, it represents the sequence \texttt{ABBACCADAA}.
	\item Normally, the sequence is much longer but to demonstrate the compression, we remove duplicate characters in order to receive a shortened sequence \texttt{ABBACADA}. Since the time interval between the first and the second \texttt{B} is greater than 30 minutes, they won't be shortened but the rest is. We now have gotten our history sequence.
	\item Since also the history sequence is very short, we say, that our search sequence consists only of the character \texttt{A} which is found at the end. 
	\item After applying the \algoSW, we receive \texttt{B} as the predicted next character of the sequence because it follows after the first found character \texttt{A} in the history sequence. Again, the algorithm operates best on much longer sequences.
\end{enumerate}

\begin{table}[h!]
	\centering
	\caption{Sorted, usable Google Location History Data}
	\begin{tabular}{cr}
		timestamps & location \\ 
		\hline
		1299998200 & Home \\ 
...
To evaluate the accuracy of the prediction, we try to make predictions on existent data to compare it to available data sets. The used data sets are comprises time periods from one to several months. Out of the whole sequence, we use a certain amount of data to be our test sequence. Hence the rest becomes the history sequence from which we extract the search sequence like before. We define all the characters after the search sequence (which are now the first few characters of the test sequence) as the \textit{expected prediction}. Now, we apply the \algoSW to the search and history sequence to obtain a \textit{possible prediction}. If a prediction exists, we compare it to the expected prediction to verify the correctness of the process. This evaluation method is depicted in Fig. \ref{fig:prediction_concept}. 

\begin{figure}[h]
	\centering
	\includegraphics[width=0.49\textwidth]{images/alignment_diagram6.pdf}
	\caption{Concept of prediction sequence testing}
	\label{fig:prediction_concept}
\end{figure}

Furthermore, we consider the impact of alteration of the search sequence length \textit{k} and the prediction sequence length \textit{l} for uncompressed (Fig. \ref{fig:prediction_uncompressed}) and compressed (Fig. \ref{fig:prediction_compressed}) sequences. For both sequences, we observe the first three predicted characters with different search sequence lengths. For the accuracy, we shift the search sequence of all data sets by a certain amount to obtain multiple results.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.49\textwidth]{images/evaluation_uncompressed.pdf}
	\caption{Predicted values from original sequence}
	\label{fig:prediction_uncompressed}
\end{figure}

As one can see, the accuracy of an uncompressed sequence rises with the search sequence length. For the 15 character search length we got 84.44\% accuracy for the first predicted character which was the best result. The accuracy for five and ten character search length were very similar and are by 76.67\% and 77.78\%.
Additionally, the accuracy for each of the three characters stays about the same within a certain range but improves overall because of the fact that the uncompressed sequence generally has multiple repetitions of the same characters.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.49\textwidth]{images/evaluation_compressed.pdf}
	\caption{Predicted values from the compressed 30 minutes sequence}
	\label{fig:prediction_compressed}
\end{figure}

Meanwhile, the values for the compressed sequence aggravate in terms of accuracy when the search sequence length is increased. The best result was for the five character search length here we got 71.11\% accuracy. For ten character search length the accuracy was 55\% and for 15 character search length the accuracy was 60\%.
 However multiple advantages are given when using this method. Trough compressing the sequence, the redundancy of pointless character repetitions is avoided so the actual prediction accuracy of useful characters is overall improved. Though it may seem, that the accuracy for uncompressed sequences is a little higher than for the compressed one, we should consider that it does not tell the real usefulness of the prediction because of random time intervals. So even if the accuracy for compressed sequences is lower, it is much more precise in that way and also faster regarding the sequence length.
