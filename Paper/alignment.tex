...
The knowledge about a person?s location plays a very important role in today?s digital world. An exact location can, in all imaginable areas, be helpful or even save lives. Another step forward is the prediction of the future location of a person. With that, one can not only acquire the information of the current location of said person, but also where he/she will with high probability be located next. To further illustrate the meaning/importance of this technology, some likely scenarios will be presented in the following.

One life saving possibility using this technology, presents itself in relation to the elderly. Fitted with the right devices, it is possible today to determine the state of health of a person and take action accordingly. Should this state take a turn for the worse or when the person has taken a fall, it can be possible to get the required help in time. In those situations the location of a person is of great importance. With the location ascertainment it will be possible to find him/her ? given, that the location information is current. Furthermore, it will be possible using location ascertainment to predict the location even if the device is forgotten or malfunctions.

An exact location is helpful in other scenarios relating to danger and rescue as well. If one imagines an accident on the freeway, it is very important for the rescue to know the exact road on which the accident took place. In this case the exact position is more important than the prediction. However, there are several situations, where the possibility of location prediction plays a more important role. Some examples of that would be rescuing hikers stuck on the trail due to bad weather, or people after a natural disaster.
 
Additional importance for location ascertainment of persons is in the field of crime fighting and -solving. It could also be a profitable for companies ? the use of this technology allows them to project and distribute personalized marketing in predetermined locations. But even a private user would find benefits in the determination of his/her location, when one bears in mind the constant rise in use of smart-home-technology. Using the location information of the user, the home system knows when he/she will be home and even the right time to turn on the heating.
...
In the following, an attempt will be made to create a location prediction using Alignments. The positioning-data used, was gathered in advance from various people and Smartphones. Using this Information and an alignment algorithm, an effort will be made to predict the next location.  
...
Our attempt is based on alignment as well. Unlike \cite{sigg2010alignment} we make us of the Smith-Waterman algorithm \cite{smith1981identification} within our location prediction approach.
...
Die Daten der CSV-Datei sind durch Semikolons voneinander getrennt. Beim Einlesen wird die Header-Zeile zunächst übersprungen. Aus den Zeilen werden Ortsbezeichnungen entnommen und anhand des timestamps sortiert. Während des zeilenweisen Einlesens werden den Orten jeweils ein Buchstabe aus dem Alphabet zugeordnet. Der Grund, warum man kann nicht einfach den ersten Buchstaben eines Ortes nehmen kann ist, dass es oft vorkommt, dass die Orte mit dem gleichen Buchstaben anfangen. Aus den Buchstaben entsteht jetzt eine sortierte Zeichenketten-Sequenz, die wir als History-Sequenz bezeichnen. 

%Das können wir so nicht schreiben!% 
%Wir sollten die Daten im ersten Schritt nicht zusammenfassen%
%Wenn doch, dann beschreiben warum das geht und wie man es machen könnte. Dabei ist die Dauer dringend zu beachten. Sprich: Es muss in den zusammengefassten Daten noch klar erkennbar sein das man acht Stunden an der Arbeit war und nur eine beim Sport.% 
%Meiner Meinung und Tests nach, sollten wir das am Ende der Implementierung als Optimierungen überlegend erwähnen.%

Im jetzigen Zustand ist die Sequenz mit Zeichen aufgebläht, die mehrfach hintereinander vorkommen können. Wie oft der gleiche Ort in der Sequenz vorkommt, interessiert uns aber an dieser Stelle nicht. Daher werden wiederholte Zeichen zu einem zusammengefasst. Dies hat zusätzlich den Effekt, dass die Sequenz kürzer wird und der Algorithmus noch effizienter darauf arbeiten kann. 
